function OptCntryExcpEditDataGridCtrlPg($scope, $filter, $modal, $document, $window, AdminService, loading, confirm, logger, formatData)
{
	var pgOptions = {moduleName : 'optCntryExcp'};
   	//save filter value after click OK button
	var oldFilterObj = {};
	//will update select options based on attrList(keys of $scope.select)
	var attrList = ["rgnCd", "subregn1Cd", "subregn2Cd", "subregn3Cd", "cntryCd", "note", "tenant", "lastModById", "lastModTs"];
	var forDisplayList = ["rgnCdForDisplay", "subregn1CdForDisplay", "subregn2CdForDisplay", "subregn3CdForDisplay", "cntryCdForDisplay", "note", "tenant", "lastModByEmail", "lastModTs"];
	//finished ajax call
	var count = 0;
	...
	$scope.update = function () {
		...
		$modal.open({templateUrl: "html/OptCntryExcpEdit.html", controller: OptCntryExcpEditCtrl,
																keyboard: false,
	...
	$scope.del = function(){
		...
		confirm.confirm(AdminService.getTipText('confirmDelete'),function(){
			...
			AdminService.deleteOptCntryExcp(options,function(result){
				...
				if(result.totalRecordAmount > 0 && result.lineData.length == 0){
					...
					AdminService.getOptCntryExcpFilteredData(para,function(result) {
						$scope.currentPage = 1;
						pgOptions.data = result.lineData;
						pgOptions.totalSize = result.totalRecordAmount;
			            serverSidePaging(AdminService,loading,confirm,$scope,$filter,pgOptions);
	...
	$scope.exportAsExcel = function(partFlag) {
		...
		//for OptCntryExcp page only ["lastModById"] is bigint
	...
	AdminService.getOptCntryExcpData(para,function(result) {
		...
		$scope.headers = [
					...
					{title: 'Teant',value: 'tenant',cd: 'tenant',visible: true,className: 'pg-col-width'},
					{title: 'Updated By',value: 'lastModByEmail',cd: 'lastModById',visible: true,className: 'pg-col-width'},
					{title: 'Updated On',value: 'lastModTs',cd: 'lastModTs',visible: false,className: ''},
		    	];
		$scope.currentPage = 1;
		pgOptions.data = result.lineData;
		pgOptions.totalSize = result.totalRecordAmount;
	...
	$scope.getFilterData = function(){
		...
		AdminService.getOptCntryExcpSelectData(function(result) {
