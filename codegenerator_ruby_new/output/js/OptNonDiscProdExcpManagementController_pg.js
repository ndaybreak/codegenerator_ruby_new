function OptNonDiscProdExcpEditDataGridCtrlPg($scope, $filter, $modal, $document, $window, AdminService, loading, confirm, logger, formatData)
{
	var pgOptions = {moduleName : 'optNonDiscProdExcp'};
   	//save filter value after click OK button
	var oldFilterObj = {};
	//will update select options based on attrList(keys of $scope.select)
	var attrList = ["rgnCd", "subregn1Cd", "subregn2Cd", "subregn3Cd", "cntryCd", "baseProd", "note", "lastModById", "lastModTs"];
	var forDisplayList = ["rgnCdForDisplay", "subregn1CdForDisplay", "subregn2CdForDisplay", "subregn3CdForDisplay", "cntryCdForDisplay", "baseProd", "note", "lastModByEmail", "lastModTs"];
	//finished ajax call
	var count = 0;
	...
	// handle Broadcast from shareService when optNonDiscProdExcpList is update in other controller
	$scope.$on('handleGridDataBroadcast', function(scope,action,result) {
		if(action == 'optNonDiscProdExcp')
		{
	...
	$scope.add = function () {
		...
		// Track user action
		_waq.push(['Click', 'OptNonDiscProdExcpManagementAdd']);
		logger.info("Executing add()");
		$modal
		.open({templateUrl: "html/OptNonDiscProdExcpEdit.html", controller: OptNonDiscProdExcpEditCtrl,
							windowClass : 'oneCol',							keyboard: false,
							resolve: {selectedOptNonDiscProdExcp: null} });
	}
	$scope.update = function () {
		...
		// Track user action
		_waq.push(['Click', 'OptNonDiscProdExcpManagementUpdate']);
		...
		$modal.open({templateUrl: "html/OptNonDiscProdExcpEdit.html", controller: OptNonDiscProdExcpEditCtrl,
																keyboard: false,
								resolve: {selectedOptNonDiscProdExcp: function() {return guidCopy;} } });
	};
	...
	$scope.del = function(){
		...
		// Track user action
		_waq.push(['Click', 'OptNonDiscProdExcpManagementDelete']);
		// get selected from scope
		var selectedItem = $scope.selection;
		if (selectedItem.length < 1) {
		...
		confirm.confirm(AdminService.getTipText('confirmDelete'),function(){
			...
			AdminService.deleteOptNonDiscProdExcp(options,function(result){
				// update optNonDiscProdExcpList when delete completed
				...
				if(result.totalRecordAmount > 0 && result.lineData.length == 0){
					...
					AdminService.getOptNonDiscProdExcpFilteredData(para,function(result) {
						$scope.currentPage = 1;
						pgOptions.data = result.lineData;
						pgOptions.totalSize = result.totalRecordAmount;
			            serverSidePaging(AdminService,loading,confirm,$scope,$filter,pgOptions);
	...
	$scope.exportAsExcel = function(partFlag) {
		...
		//gridDataToJson(setting, 'Non-discountable Product Exception');
		//for OptNonDiscProdExcp page only ["lastModById"] is bigint
		...
		var entityName = 'OptNonDiscProdExcp';
	...
	AdminService.getOptNonDiscProdExcpData(para,function(result) {
	...
	$scope.getFilterData = function(){
		...
		AdminService.getOptNonDiscProdExcpSelectData(function(result) {
