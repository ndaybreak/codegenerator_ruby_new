function OptPromoEditDataGridCtrlPg($scope, $filter, $modal, $document, $window, AdminService, loading, confirm, logger, formatData)
{
	var pgOptions = {moduleName : 'optPromo'};
   	//save filter value after click OK button
	var oldFilterObj = {};
	//will update select options based on attrList(keys of $scope.select)
	var attrList = ["routeToMarket", "region", "subregn1Cd", "subregn2Cd", "subregn3Cd", "cntryCd", "busGrpId", "busModelCd", "note", "lastModById", "lastModTs"];
	var forDisplayList = ["routeToMarket", "rgnCdForDisplay", "subregn1CdForDisplay", "subregn2CdForDisplay", "subregn3CdForDisplay", "cntryCdForDisplay", "busGrpForDisplay", "busModelForDisplay", "note", "lastModByEmail", "lastModTs"];
	//finished ajax call
	var count = 0;
	...
	// handle Broadcast from shareService when optPromoList is update in other controller
	...
	$scope.add = function () {
		...
		// Track user action
		_waq.push(['Click', 'OptPromoManagementAdd']);
		logger.info("Executing add()");
		$modal
		.open({templateUrl: "html/OptPromoEdit.html", controller: OptPromoEditCtrl,
							windowClass : 'oneCol',							keyboard: false,
							resolve: {selectedOptPromo: null} });
	}
	$scope.update = function () {
		...
		// Track user action
		_waq.push(['Click', 'OptPromoManagementUpdate']);
		...
		$modal.open({templateUrl: "html/OptPromoEdit.html", controller: OptPromoEditCtrl,
																keyboard: false,
								resolve: {selectedOptPromo: function() {return guidCopy;} } });
	};
	...
	$scope.del = function(){
		...
		// Track user action
		_waq.push(['Click', 'OptPromoManagementDelete']);
		// get selected from scope
		var selectedItem = $scope.selection;
		if (selectedItem.length < 1) {
		...
		confirm.confirm(AdminService.getTipText('confirmDelete'),function(){
			...
			AdminService.deleteOptPromo(options,function(result){
				// update optPromoList when delete completed
				...
				if(result.totalRecordAmount > 0 && result.lineData.length == 0){
					...
					AdminService.getOptPromoFilteredData(para,function(result) {
						$scope.currentPage = 1;
						pgOptions.data = result.lineData;
						pgOptions.totalSize = result.totalRecordAmount;
			            serverSidePaging(AdminService,loading,confirm,$scope,$filter,pgOptions);
	...
	$scope.exportAsExcel = function(partFlag) {
		...
		//gridDataToJson(setting, 'Promo pricing');
		//for OptPromo page only ["lastModById"] is bigint
	...
	AdminService.getOptPromoData(para,function(result) {
		AdminService.getStaticData($scope.setAuth,plusOne);
		$scope.headers = [
					{title: '',value: '',cd: '',visible: true,className: ''},
					{title: 'Route to Market',value: 'routeToMarket',cd: 'routeToMarket',visible: true,className: 'pg-col-width'},
					...
					{title: 'Business Group',value: 'busGrpForDisplay',cd: 'busGrpId',visible: true,className: 'pg-col-width'},
					{title: 'Business Model',value: 'busModelForDisplay',cd: 'busModelCd',visible: true,className: 'pg-col-width'},
	...
	$scope.getFilterData = function(){
		...
		AdminService.getOptPromoSelectData(function(result) {
