function OptCustSegExcpEditDataGridCtrlPg($scope, $filter, $modal, $document, $window, AdminService, loading, confirm, logger, formatData)
{
	var pgOptions = {moduleName : 'optCustSegExcp'};
   	//save filter value after click OK button
	var oldFilterObj = {};
	//will update select options based on attrList(keys of $scope.select)
	var attrList = ["rgnCd", "subregn1Cd", "subregn2Cd", "subregn3Cd", "cntryCd", "custSeg", "note", "lastModById", "lastModTs"];
	var forDisplayList = ["rgnCdForDisplay", "subregn1CdForDisplay", "subregn2CdForDisplay", "subregn3CdForDisplay", "cntryCdForDisplay", "custSegForDisplay", "note", "lastModByEmail", "lastModTs"];
	//finished ajax call
	var count = 0;
	...
	// handle Broadcast from shareService when optCustSegExcpList is update in other controller
	$scope.$on('handleGridDataBroadcast', function(scope,action,result) {
		if(action == 'optCustSegExcp')
		{
	...
	$scope.add = function () {
		...
		// Track user action
		_waq.push(['Click', 'OptCustSegExcpManagementAdd']);
		logger.info("Executing add()");
		$modal
		.open({templateUrl: "html/OptCustSegExcpEdit.html", controller: OptCustSegExcpEditCtrl,
							windowClass : 'oneCol',							keyboard: false,
							resolve: {selectedOptCustSegExcp: null} });
	}
	$scope.update = function () {
		...
		// Track user action
		_waq.push(['Click', 'OptCustSegExcpManagementUpdate']);
		...
		$modal.open({templateUrl: "html/OptCustSegExcpEdit.html", controller: OptCustSegExcpEditCtrl,
																keyboard: false,
								resolve: {selectedOptCustSegExcp: function() {return guidCopy;} } });
	};
	...
	$scope.del = function(){
		...
		// Track user action
		_waq.push(['Click', 'OptCustSegExcpManagementDelete']);
		// get selected from scope
		var selectedItem = $scope.selection;
		if (selectedItem.length < 1) {
		...
		confirm.confirm(AdminService.getTipText('confirmDelete'),function(){
			...
			AdminService.deleteOptCustSegExcp(options,function(result){
				// update optCustSegExcpList when delete completed
				...
				if(result.totalRecordAmount > 0 && result.lineData.length == 0){
					...
					AdminService.getOptCustSegExcpFilteredData(para,function(result) {
						$scope.currentPage = 1;
						pgOptions.data = result.lineData;
						pgOptions.totalSize = result.totalRecordAmount;
			            serverSidePaging(AdminService,loading,confirm,$scope,$filter,pgOptions);
	...
	$scope.exportAsExcel = function(partFlag) {
		...
		//gridDataToJson(setting, 'Customer Segment Exception');
		//for OptCustSegExcp page only ["lastModById"] is bigint
	...
	AdminService.getOptCustSegExcpData(para,function(result) {
		...
		$scope.headers = [
					...
					{title: 'Customer Segment',value: 'custSegForDisplay',cd: 'custSeg',visible: true,className: 'pg-col-width'},
	...
	$scope.getFilterData = function(){
		...
		AdminService.getOptCustSegExcpSelectData(function(result) {
