function OptCustExcpEditDataGridCtrlPg($scope, $filter, $modal, $document, $window, AdminService, loading, confirm, logger, formatData)
{
	var pgOptions = {moduleName : 'optCustExcp'};
   	//save filter value after click OK button
	var oldFilterObj = {};
	//will update select options based on attrList(keys of $scope.select)
	var attrList = ["rgnCd", "subregn1Cd", "subregn2Cd", "subregn3Cd", "cntryCd", "amid2", "mdcpOrgId", "mdcpSiteId", "note", "lastModById", "lastModTs"];
	var forDisplayList = ["rgnCdForDisplay", "subregn1CdForDisplay", "subregn2CdForDisplay", "subregn3CdForDisplay", "cntryCdForDisplay", "amid2ForDisplay", "mdcpOrgId", "mdcpSiteId", "note", "lastModByEmail", "lastModTs"];
	//finished ajax call
	var count = 0;
	...
	var inputAndDatePickerBoxs = ["amid2", "mdcpOrgId", "mdcpSiteId", "lastModTs"];
	// handle Broadcast from shareService when optCustExcpList is update in other controller
	$scope.$on('handleGridDataBroadcast', function(scope,action,result) {
		if(action == 'optCustExcp')
		{
	...
	$scope.add = function () {
		...
		// Track user action
		_waq.push(['Click', 'OptCustExcpManagementAdd']);
		logger.info("Executing add()");
		$modal
		.open({templateUrl: "html/OptCustExcpEdit.html", controller: OptCustExcpEditCtrl,
							windowClass : 'oneCol',							keyboard: false,
							resolve: {selectedOptCustExcp: null} });
	}
	$scope.update = function () {
		...
		// Track user action
		_waq.push(['Click', 'OptCustExcpManagementUpdate']);
		...
		$modal.open({templateUrl: "html/OptCustExcpEdit.html", controller: OptCustExcpEditCtrl,
																keyboard: false,
								resolve: {selectedOptCustExcp: function() {return guidCopy;} } });
	};
	...
	$scope.del = function(){
		...
		// Track user action
		_waq.push(['Click', 'OptCustExcpManagementDelete']);
		// get selected from scope
		var selectedItem = $scope.selection;
		if (selectedItem.length < 1) {
		...
		confirm.confirm(AdminService.getTipText('confirmDelete'),function(){
			...
			AdminService.deleteOptCustExcp(options,function(result){
				// update optCustExcpList when delete completed
				...
				if(result.totalRecordAmount > 0 && result.lineData.length == 0){
					...
					AdminService.getOptCustExcpFilteredData(para,function(result) {
						$scope.currentPage = 1;
						pgOptions.data = result.lineData;
						pgOptions.totalSize = result.totalRecordAmount;
			            serverSidePaging(AdminService,loading,confirm,$scope,$filter,pgOptions);
	...
	$scope.exportAsExcel = function(partFlag) {
		...
		//gridDataToJson(setting, 'Customer Exception');
		//for OptCustExcp page only ["lastModById"] is bigint
	...
	AdminService.getOptCustExcpData(para,function(result) {
		...
		$scope.headers = [
					...
					{title: 'MDCP Site ID',value: 'mdcpSiteId',cd: 'mdcpSiteId',visible: true,className: 'pg-col-width'},
	...
	$scope.getFilterData = function(){
		...
		AdminService.getOptCustExcpSelectData(function(result) {
