function OptCurrExcpEditDataGridCtrlPg($scope, $filter, $modal, $document, $window, AdminService, loading, confirm, logger, formatData)
{
	var pgOptions = {moduleName : 'optCurrExcp'};
   	//save filter value after click OK button
	var oldFilterObj = {};
	//will update select options based on attrList(keys of $scope.select)
	var attrList = ["rgnCd", "subregn1Cd", "subregn2Cd", "subregn3Cd", "cntryCd", "currCd", "note", "lastModById", "lastModTs"];
	var forDisplayList = ["rgnCdForDisplay", "subregn1CdForDisplay", "subregn2CdForDisplay", "subregn3CdForDisplay", "cntryCdForDisplay", "currCd", "note", "lastModByEmail", "lastModTs"];
	//finished ajax call
	var count = 0;
	...
	$scope.update = function () {
		...
		$modal.open({templateUrl: "html/OptCurrExcpEdit.html", controller: OptCurrExcpEditCtrl,
																keyboard: false,
	...
	$scope.del = function(){
		...
		confirm.confirm(AdminService.getTipText('confirmDelete'),function(){
			...
			AdminService.deleteOptCurrExcp(options,function(result){
				...
				if(result.totalRecordAmount > 0 && result.lineData.length == 0){
					...
					AdminService.getOptCurrExcpFilteredData(para,function(result) {
						$scope.currentPage = 1;
						pgOptions.data = result.lineData;
						pgOptions.totalSize = result.totalRecordAmount;
			            serverSidePaging(AdminService,loading,confirm,$scope,$filter,pgOptions);
	...
	$scope.exportAsExcel = function(partFlag) {
		...
		//for OptCurrExcp page only ["lastModById"] is bigint
	...
	AdminService.getOptCurrExcpData(para,function(result) {
		...
		$scope.headers = [
					...
					{title: 'Currency',value: 'currCd',cd: 'currCd',visible: true,className: 'pg-col-width'},
	...
	$scope.getFilterData = function(){
		...
		AdminService.getOptCurrExcpSelectData(function(result) {
