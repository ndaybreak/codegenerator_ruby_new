function OptProdExcpEditDataGridCtrlPg($scope, $filter, $modal, $document, $window, AdminService, loading, confirm, logger, formatData)
{
	var pgOptions = {moduleName : 'optProdExcp'};
   	//save filter value after click OK button
	var oldFilterObj = {};
	//will update select options based on attrList(keys of $scope.select)
	var attrList = ["rgnCd", "subregn1Cd", "subregn2Cd", "subregn3Cd", "cntryCd", "prodLnId", "prodFmlyId", "baseProd", "note", "lastModById", "lastModTs"];
	var forDisplayList = ["rgnCdForDisplay", "subregn1CdForDisplay", "subregn2CdForDisplay", "subregn3CdForDisplay", "cntryCdForDisplay", "prodLnIdForDisplay", "prodFmlyIdForDisplay", "baseProd", "note", "lastModByEmail", "lastModTs"];
	//finished ajax call
	var count = 0;
	...
	var inputAndDatePickerBoxs = ["baseProd", "lastModTs"];
	// handle Broadcast from shareService when optProdExcpList is update in other controller
	$scope.$on('handleGridDataBroadcast', function(scope,action,result) {
		if(action == 'optProdExcp')
		{
	...
	$scope.add = function () {
		...
		// Track user action
		_waq.push(['Click', 'OptProdExcpManagementAdd']);
		logger.info("Executing add()");
		$modal
		.open({templateUrl: "html/OptProdExcpEdit.html", controller: OptProdExcpEditCtrl,
							windowClass : 'oneCol',							keyboard: false,
							resolve: {selectedOptProdExcp: null} });
	}
	$scope.update = function () {
		...
		// Track user action
		_waq.push(['Click', 'OptProdExcpManagementUpdate']);
		...
		$modal.open({templateUrl: "html/OptProdExcpEdit.html", controller: OptProdExcpEditCtrl,
																keyboard: false,
								resolve: {selectedOptProdExcp: function() {return guidCopy;} } });
	};
	...
	$scope.del = function(){
		...
		// Track user action
		_waq.push(['Click', 'OptProdExcpManagementDelete']);
		// get selected from scope
		var selectedItem = $scope.selection;
		if (selectedItem.length < 1) {
		...
		confirm.confirm(AdminService.getTipText('confirmDelete'),function(){
			...
			AdminService.deleteOptProdExcp(options,function(result){
				// update optProdExcpList when delete completed
				...
				if(result.totalRecordAmount > 0 && result.lineData.length == 0){
					...
					AdminService.getOptProdExcpFilteredData(para,function(result) {
						$scope.currentPage = 1;
						pgOptions.data = result.lineData;
						pgOptions.totalSize = result.totalRecordAmount;
			            serverSidePaging(AdminService,loading,confirm,$scope,$filter,pgOptions);
	...
	$scope.exportAsExcel = function(partFlag) {
		...
		//gridDataToJson(setting, 'Product Exception');
		//for OptProdExcp page only ["lastModById"] is bigint
	...
	AdminService.getOptProdExcpData(para,function(result) {
		...
		$scope.headers = [
					...
					{title: 'Product Line',value: 'prodLnIdForDisplay',cd: 'prodLnId',visible: true,className: 'pg-col-width'},
					{title: 'Product Family',value: 'prodFmlyIdForDisplay',cd: 'prodFmlyId',visible: true,className: 'pg-col-width'},
					{title: 'Base Product',value: 'baseProd',cd: 'baseProd',visible: true,className: 'pg-col-width'},
	...
	$scope.getFilterData = function(){
		...
		AdminService.getOptProdExcpSelectData(function(result) {
